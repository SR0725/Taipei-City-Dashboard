...
# 轉換器：TWD97 TM2 → WGS84
# 台灣區常用 TM2（121經線帶），EPSG:3826 → EPSG:4326 (WGS84)
...
with engine.connect() as conn:
    print("🗑️ Truncating table: taipei_mrt_station ...")
    conn.execute(text("TRUNCATE TABLE taipei_mrt_station"))
    conn.commit()

# Insert new data
print("💾 Inserting new data ...")
df.to_sql("taipei_mrt_station", con=engine, if_exists='append', index=False)

print(f"🎉 Done! Total {len(df)} records saved to table: taipei_mrt_station 🚀✨")
